...
- [x] **Planner page** (`src/app/planner/page.tsx`) — vertical stack of day cards (one per day of the visible week), each split into Morning / Afternoon / Evening sub-sections. Auto-aggregates from 5 sources: logged MA, planned MA, logged lifts, logged cardio, and `weekly_plans` (custom entries with explicit day_part). Tap "+ Add" in any slot to open an overlay sheet (MA class-pick / Cardio / Custom). **Time bucketing**: one shared rule (`bucketByTime`, boundaries `MORNING_END="11:00"` / `EVENING_START="16:30"`) decides BOTH where an entry displays AND which Elevate classes a slot's picker offers — so anything added to a slot reappears in that slot, with no class falling through a gap (fixes 10:30 Sat Kickboxing). Matches the user's mental model: 11am→afternoon, 4:30pm→evening. Cardio + lift inserts pre-fill `start_time` from the slot (`defaultStartTimeForSlot`: 09:00/13:00/18:00). Lifts bucket by `start_time` (NULL→Morning fallback). **Cards are tappable**: cardio/custom open an in-place edit sheet (edit activity/duration/time/intensity/notes, or title/day-part; with a Delete button); lift cards link to `/lift`, MA + planned cards link to `/martial-arts`. The accidental-delete `×` was removed from cards (it permanently deleted real sessions+notes with only a vague "Delete X?" confirm — a likely cause of earlier data loss); deletes now live behind explicit, clearly-worded confirms.
...
- start_time (time, nullable) — set by the Planner when a lift is added to a specific morning/afternoon/evening slot, so it buckets back there. Lifts logged from the Lift tab leave it NULL (Planner falls back to Morning). Added via `scripts/add-lift-start-time.sql`.
- created_at
